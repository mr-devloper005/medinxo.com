...
const mediaDistributionTaskAliases = [
  'mediaDistribution',
  'mediadistribution',
  'media-distribution',
  'media_distribution',
...
const getPostType = (post: SitePost) => {
  const content = getContent(post)
  const explicit = asText(content.type)
  if (explicit) return normalizeCategory(explicit)
  const knownAlias = post.tags?.find((tag) => mediaDistributionTaskAliases.includes(normalizeCategory(tag) as (typeof mediaDistributionTaskAliases)[number]))
  return knownAlias ? normalizeCategory(knownAlias) : ''
}
...
const isRealMediaDistributionPost = (post: SitePost) => {
  if (!isPublishedPost(post)) return false
  const postType = getPostType(post)
  if (!postType) return true
  return mediaDistributionTaskAliases.includes(postType as (typeof mediaDistributionTaskAliases)[number])
}

const uniqueBySlugOrId = (posts: SitePost[]) =>
  Array.from(new Map(posts.filter((post) => post?.slug || post?.id).map((post) => [post.slug || post.id, post])).values())

async function fetchEditableMediaDistributionPosts({
  page,
  limit,
  category,
  primaryPosts,
  ...
  limit: number
  category: string
  primaryPosts: SitePost[]
  primaryPagination: SiteFeedPagination
}): Promise<{ posts: SitePost[]; pagination: SiteFeedPagination }> {
  const normalizedCategory = category === 'all' ? '' : normalizeCategory(category)
  const aliasFeeds = await Promise.all(
    mediaDistributionTaskAliases.map((alias) =>
      fetchSiteFeed<SitePost>(100, { task: alias, fresh: true, timeoutMs: 5000 }).catch(() => null)
    )
  )
  const unscopedFeed = await fetchSiteFeed<SitePost>(1000, { fresh: true, timeoutMs: 5000 }).catch(() => null)
  const merged = uniqueBySlugOrId([
    ...primaryPosts,
    ...aliasFeeds.flatMap((feed) => feed?.posts || []),
    ...(unscopedFeed?.posts || []),
  ]).filter((post) => {
    if (!isRealMediaDistributionPost(post)) return false
    if (!normalizedCategory) return true
    const postCategory = normalizeCategory(getCategory(post, ''))
    const tags = post.tags?.map((tag) => normalizeCategory(tag)) || []
    return postCategory === normalizedCategory || tags.includes(normalizedCategory)
  })

  if (!merged.length) {
    return { posts: primaryPosts, pagination: primaryPagination }
  }

  const start = (page - 1) * limit
  return {
    posts: merged.slice(start, start + limit),
    pagination: fallbackPagination(page, limit, merged.length),
  }
}
...
export async function EditableTaskArchiveRoute({
  ...
  const limit = 24
  const fetched = await fetchPaginatedTaskPosts(task, { page, limit, category, fresh: true })
  const { posts, pagination } = task === 'mediaDistribution'
    ? await fetchEditableMediaDistributionPosts({
      page,
      limit,
      category,
      primaryPosts: fetched.posts,
      primaryPagination: fetched.pagination,
    })
    : fetched
...
function EditorialArchive({
  ...
  const remaining = posts.length > 3 ? posts.slice(3) : posts

  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-[#f6f5f7] text-[#1c1b24]">
        <section className="bg-white">
          <div className="mx-auto flex max-w-[1240px] flex-col gap-6 px-4 py-10 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8 lg:py-14">
            <div>
              <p className="inline-flex rounded-full border border-black/10 px-4 py-2 text-xs font-black uppercase tracking-[0.24em] text-[var(--slot4-accent)]">Media distribution archive</p>
            ...
            <p className="max-w-md border-l-4 border-[var(--slot4-accent)] pl-5 text-sm font-bold leading-7 text-black/65">
              Real media distribution posts, publisher-ready summaries, and campaign tasks organized for fast discovery.
            </p>
          </div>
        </section>

        <section className="bg-[#07050d] text-white">
          <div className="mx-auto flex max-w-[1240px] gap-7 overflow-x-auto px-4 py-4 text-xs font-black uppercase tracking-[0.16em] sm:px-6 lg:px-8">
            <Link href={basePath} className={category === 'all' ? 'text-[#d7ff66]' : 'hover:text-[#d7ff66]'}>Latest</Link>
            {categories.slice(0, 8).map((item) => (
              <Link key={item.slug} href={pageHref(basePath, item.slug, 1)} className={category === item.slug ? 'text-[#d7ff66]' : 'whitespace-nowrap hover:text-[#d7ff66]'}>
                {item.name}
              </Link>
            ))}
          </div>
        ...
        {lead ? (
          <section className="mx-auto grid max-w-[1240px] gap-5 px-4 py-8 sm:px-6 lg:grid-cols-[1.75fr_0.75fr] lg:px-8">
            <Link href={`${basePath}/${lead.slug}`} className="group editable-card relative min-h-[34rem] overflow-hidden bg-black">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                ...
                <h2 className="mt-5 max-w-4xl text-4xl font-black leading-[0.98] tracking-[-0.055em] sm:text-6xl">{lead.title}</h2>
                
              </div>
            </Link>
            <div className="grid">
              <div className="editable-card bg-[var(--slot4-accent)] p-6 text-white">
                <p className="text-xs font-black uppercase tracking-[0.24em]">Top stories</p>
                <p className="mt-3 text-3xl font-black leading-tight tracking-[-0.05em]">What the distribution desk is watching now.</p>
              </div>
              {secondary.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group editable-card grid grid-cols-[7rem_1fr] bg-white">
                  <img src={getImage(post)} alt="" className="h-full min-h-40 w-full object-cover transition group-hover:scale-[1.02]" />
                  <div className="p-5">
        ...
        <section className="mx-auto max-w-[1240px] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5 pb-4">
            <h2 className="text-4xl font-black tracking-[-0.055em] sm:text-5xl">More from the desk</h2>
            <form action={basePath} className="flex overflow-hidden rounded-full bg-white shadow-sm">
          ...
          {remaining.length ? (
            <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group editable-card overflow-hidden border border-black/10 bg-white">
                  ...
                  <div className="p-5">
                    ...
                    <h3 className="mt-4 text-2xl font-black leading-[1.05] tracking-[-0.045em]">{post.title}</h3>
            ...
            <div className="border border-dashed border-black bg-white p-12 text-center">
              ...
              <h2 className="mt-4 text-3xl font-black tracking-[-0.05em]">No stories found</h2>
