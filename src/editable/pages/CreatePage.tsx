...
const fieldClass = 'rounded-2xl border border-black/10 bg-[#f0eef3] px-4 py-3 text-sm font-bold text-[#1c1b24] outline-none transition placeholder:text-current/35 focus:border-[#6b00f5] focus:bg-white'
...
export default function CreatePage() {
  ...
  if (!session) {
    return (
      <EditableSiteShell>
        <main className="min-h-screen bg-[#f6f5f7] px-4 py-16 text-[#1c1b24] sm:px-6 lg:px-8">
          <section className="mx-auto grid max-w-5xl gap-8 rounded-[32px] bg-white p-7 shadow-[0_30px_90px_rgba(20,14,35,.08)] md:grid-cols-[0.9fr_1.1fr] md:p-10">
            <div className="flex h-full min-h-72 items-center justify-center rounded-[28px] bg-[#07050d] text-white">
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[#f6f5f7] text-[#1c1b24]">
        <section className="mx-auto max-w-[1240px] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-[32px] bg-white p-6 shadow-[0_30px_90px_rgba(20,14,35,.08)] backdrop-blur lg:grid-cols-[0.85fr_1.15fr] lg:p-10">
            <aside>
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-2">
                {enabledTasks.map((item) => {
                  ...
                  return (
                    <button key={item.key} type="button" onClick={() => setTask(item.key)} className={`rounded-2xl border p-4 text-left transition ${active ? 'border-[#6b00f5] bg-[#6b00f5] text-white' : 'border-black/10 bg-[#f7f5fb] hover:-translate-y-0.5'}`}>
            ...
            <form onSubmit={submit} className="rounded-[28px] bg-[#f7f5fb] p-5 sm:p-7">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <span className="rounded-full bg-white px-4 py-2 text-xs font-black uppercase tracking-[0.16em] text-[var(--slot4-accent)]">{session.name}</span>
              </div>
              ...
              <button type="submit" className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 rounded-full bg-[var(--slot4-accent)] px-6 text-sm font-black uppercase tracking-[0.18em] text-white transition hover:-translate-y-0.5 hover:bg-black">
